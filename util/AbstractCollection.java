 ...
 * method.
 * <p>
 * 实现一个可修改的集合,程序员必须另外覆盖这个类的add方法(否则抛出UnsupportedOperationException)方式,
 * 和迭代器返回的迭代器方法必须另外实现其remove方法。
 * 
...
public abstract class AbstractCollection<E> implements Collection<E> {
	 ...
	 * 如果该集合包含指定的元素，则返回true。更正式地说，如果这个集合包含至少一个元素e，那么返回true，这样（o==null？e = =
	 * null:o.equals(e))。 这个实现迭代了集合中的元素，检查每个元素，以实现与指定元素的相等。
	 * 
	 ...
	 * 
	 * 返回的数组将是“安全的”，因为这个集合没有对它进行任何引用。（换句话说，这个方法必须分配一个新的数组，即使这个集合是由数组支持的）。因此，
	 * 调用者可以自由地修改返回的数组。
	 * 
	 ...
	 * 
	 * 要分配的数组的最大大小。一些vm在数组中保留一些标头词。 分配较大数组的尝试可能导致OutOfMemoryError:请求的数组大小超过VM限制。
	 */
	 ...
	 * iterator. 当迭代器返回比预期更多的元素，并从迭代器中完成填充时，重新分配在toArray中使用的数组。
	 * 
	 * @param r
	 *            the array, replete with previously stored elements
	 *            数组，包含先前存储的元素。
	 * @param it
	 *            the in-progress iterator over this collection 它正在进行的迭代器在这个集合
	 ...
	 * 这个实现在集合中迭代查找指定的元素。如果它找到了元素，则使用迭代器的remove方法从集合中删除元素。
	 * <p>
	 * Note that this implementation throws an
	 * <tt>UnsupportedOperationException</tt> if the iterator returned by this
	 * collection's iterator method does not implement the <tt>remove</tt>
	 * method and this collection contains the specified object.
	 * 请注意,此实现抛出UnsupportedOperationException如果方式这个集合的迭代器方法返回的迭代器没有实现remove方法,
	 * 这个集合包含指定的对象。
	 * 
	 * @throws UnsupportedOperationException
	 *             {@inheritDoc}
	 ...
	 * otherwise <tt>false</tt>. 如果该集合包含指定集合中的所有元素，那么返回true。
	 * 这个实现在指定的集合上迭代，检查迭代器返回的每个元素，看看它是否包含在这个集合中。如果所有元素都包含true，则返回，否则是false。
	 * 
	 * @throws ClassCastException
	 *             {@inheritDoc}
	 * @throws NullPointerException
	 ...
	 * 该实现在指定的集合上迭代，并将迭代器返回的每个对象添加到这个集合中。
